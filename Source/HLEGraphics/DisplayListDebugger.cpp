		...
		switch (gDebugTask)
		{
			...
			case kTaskScrub:
			{
				...
				for (u32 i = 0; i < 8; ++i)
				{
					...
					if ((tile.format != 0 || tile.size != 0) && tile_size.GetWidth() > 0 && tile_size.GetHeight() > 0)
					{
			...
			case kTaskTakeScreenshot:
			{
				...
				PngSaveImage(connection, pixels, pitch, width, height, false);
