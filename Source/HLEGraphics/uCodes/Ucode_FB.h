...
{
	u16* pixels = (u16*)malloc(FB_WIDTH * FB_HEIGHT * sizeof(u16));  // TODO: should cache this, but at some point we'll
																	 // need to deal with variable framebuffer size, so
																	 // do this later.
	u32 src_offset = 0;
	...
	for (u32 y = 0; y < FB_HEIGHT; ++y)
	{
		...
		{
			pixels[dst_offset] = (gu8RamBase[(origin + src_offset) ^ U8_TWIDDLE] << 8) |
								 gu8RamBase[(origin + src_offset + 1) ^ U8_TWIDDLE] |
