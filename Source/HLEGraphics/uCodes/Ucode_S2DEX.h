	...
	else
	{
		...
		switch (txtr->block.type)
		{
			...
			case S2DEX_OBJLT_TXTRTILE:
				...
				ti.SetPitch((sprite->imageSiz == G_IM_SIZ_4b) ? (ti.GetWidth() >> 1)
...
static inline void Draw_ObjSprite(const uObjSprite* sprite, ESpriteMode mode, const CNativeTexture* texture)
{
	...
	switch (mode)
	{
		...
		case NO_ROTATION:
			...
			// Used by Worms
			if (sprite->imageFlags & 1) Swap<f32>(x0, x1);

			if (sprite->imageFlags & 0x10) Swap<f32>(y0, y1);
...
inline void DLParser_Yoshi_MemRect(MicroCodeCommand command)
{
	...
	if (y1 > scissors.bottom) y1 = scissors.bottom;
...
void DLParser_OB_YUV(const uObjSprite* sprite)
{
	...
	if ((ul_x >= ci_width) || (ul_y >= ci_height)) return;
...
void DLParser_S2DEX_ObjRendermode(MicroCodeCommand command) { DL_PF("    S2DEX_ObjRendermode (Ignored)"); }

void DLParser_S2DEX_SelectDl(MicroCodeCommand command) { DL_PF("    S2DEX_SelectDl (Ignored)"); }
...
void DLParser_S2DEX_BgCopy(MicroCodeCommand command)
{
	...
	gRenderer->Draw2DTexture((float)frameX, (float)frameY, (float)frameW, (float)frameH, (float)imageX, (float)imageY,
...
void DLParser_S2DEX_Bg1cyc(MicroCodeCommand command)
{
	...
	ti.SetPitch(((objBg->imageW / 4 << ti.GetSize() >> 1) >> 3)
