...
class CProfileCallstack
{
   public:
	CProfileCallstack(const CProfileCallstack* parent, const std::vector<CProfileItem*>& items)
		: mParent(parent), mItems(items), mHash(ComputeCallstackHash(items)), mTotalTime(0), mStartTime(0), mHitCount(0)
	{
	}
	...
	s32 Compare(const CProfileCallstack& rhs) const
	{
		...
		//
		if (i >= mItems.size() && i >= rhs.mItems.size()) return 0;
...
{
	bool operator()(const CProfileCallstack* a, const CProfileCallstack* b) { return a->Compare(*b) > 0; }
};
...
template <>
bool CSingleton<CProfiler>::Create()
{
...
SProfileItemHandle CProfiler::AddItem(const char* p_str) { return mpImpl->AddItem(p_str); }
