...
class CMemoryPool
{
   ...
   public:
	CMemoryPool() : ItemSize((sizeof(T) + (Align - 1)) & ~(Align - 1)), mMemory(new u8[MaxItems * ItemSize])
	{
		Chunk* first(NULL);
	...
	{
		DAEDALUS_ASSERT(ptr == NULL || ptr >= mMemory && ptr < &mMemory[MaxItems * ItemSize],
...
template <>
class CMemoryPoolAllocator<void>
{
   public:
	...
	struct rebind
	{
		typedef CMemoryPoolAllocator<U> other;
	};
};
...
class CMemoryPoolAllocator
{
   public:
	...
	template <typename U>
	CMemoryPoolAllocator(const CMemoryPoolAllocator<U>&)
	{
	}
...
template <typename T>
CMemoryPool<T> CMemoryPoolAllocator<T>::mPool;
