...
struct SChunkInfo
{
	...
	bool ContainsAddress(u32 address) const { return address >= StartOffset && address < StartOffset + CHUNK_SIZE; }

	bool InUse() const { return StartOffset != INVALID_ADDRESS; }
};

ROMFileCache::ROMFileCache() : mpROMFile(NULL), mChunkMapEntries(0), mpChunkMap(NULL), mMRUIdx(0)
{
	CHUNK_SIZE = 2 * 1024;
	CACHE_SIZE = 1024;
	...
	DAEDALUS_ASSERT((1 << (sizeof(CacheIdx) * 8)) > CACHE_SIZE,
					"Need to increase size of CacheIdx typedef to allow sufficient entries to be indexed");
...
inline u32 GetChunkStartAddress(u32 address) { return (address / CHUNK_SIZE) * CHUNK_SIZE; }
