	...
	while (num--)
	{
		...
		switch (type)
		{
			...
			case 0xA0:
				...
				if ((code->orig == CHEAT_CODE_MAGIC_VALUE) && code->orig) code->orig = *(u8*)(p_mem);
			...
			case 0xA1:
				// Check if orig value is initialized and valid to store current value
				if ((code->orig == CHEAT_CODE_MAGIC_VALUE) && code->orig) code->orig = *(u16*)(p_mem);
			...
			case 0x89:
				...
				break;
			case 0x04:
				if (((code->addr >> 20) & 0xF) == 0x5) Memory_AI_SetRegister(code->addr & 0x0FFFFFFF, value);
				break;
			case 0x50:
			{
				skip = true;
...
bool CheatCodes_Read(const char* rom_name, const char* file, u8 countryID)
{
	...
	if (bfound)
	{
		...
		codegroupcount = 0;
		// TODO(strmnnrmn): This read looks incredibly unsafe.
		while (codegroupcount < numberofgroups && fgets(buffer, 32767, stream) &&
			   strlen(buffer) > 8)  // 32767 makes sure the entire line is read
		{
			// Codes for the group are in the string buffer[]
			for (c1 = 0; buffer[c1] != '=' && buffer[c1] != '\0'; c1++)
				codegrouplist[codegroupcount].name[c1] = buffer[c1];
			...
			for (c2 = 0; c2 < (strlen(buffer) - c1 - 1) / 14; c2++, codegrouplist[codegroupcount].codecount++)
			{
				if (c2 < MAX_CHEATCODE_PER_ENTRY)
				{
					...
					if (c2 > 0 && ((codegrouplist[codegroupcount].codelist[c2 - 1].addr >> 24) & 0xFF) == 0x50)
					{
						...
						// Then we can convert it to a normal cheat code by just filling each slot with the repeater
						// counter, but it would alot of memory..  The way compressed cheat codes work is in pairs, the
						// serial repeater 0x50xxx and the actual code 0x80/81xxx; ex 50001801-0001,80118444-0000  For now
						// if previous cheatcode was compressed, don't preswap
					}
					else
					{
						// Pre-swap address
