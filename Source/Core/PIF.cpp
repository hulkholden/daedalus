...
class IController : public CController
{
  ...
  private:
	...
	bool IsEepromPresent() const
	{
		return mpEepromData != NULL;
	}
...
template <>
bool CSingleton<CController>::Create()
{
...
IController::IController() : mpPifRam(NULL), mpEepromData(NULL)
{
#ifdef DAEDALUS_DEBUG_PIF
...
void IController::Process()
{
	...
	{
		DPF_PIF("0x%02x%02x%02x%02x : 0x%02x%02x%02x%02x  |  0x%02x%02x%02x%02x : 0x%02x%02x%02x%02x", mpInput[(x + 0)],
		        mpInput[(x + 1)], mpInput[(x + 2)], mpInput[(x + 3)], mpInput[(x + 4)], mpInput[(x + 5)],
		        mpInput[(x + 6)], mpInput[(x + 7)], mpPifRam[(x + 0)], mpPifRam[(x + 1)], mpPifRam[(x + 2)],
		        mpPifRam[(x + 3)], mpPifRam[(x + 4)], mpPifRam[(x + 5)], mpPifRam[(x + 6)], mpPifRam[(x + 7)]);
	}
	DPF_PIF("");
	DPF_PIF("");
	DPF_PIF("**                                         **");
...
bool IController::ProcessController(u8* cmd, u32 channel)
{
	...
	// From the patent, it says that if a controller is plugged in and the memory card is removed, the CONT_CARD_PULL
	// flag will be set. Cleared if CONT_RESET or CONT_GET_STATUS is issued. Might need to set this if
	// mContMemPackPresent is false?
...
void IController::n64_cic_nus_6105()
{
	...
	char lut0[0x10] = {0x4, 0x7, 0xA, 0x7, 0xE, 0x5, 0xE, 0x1, 0xC, 0xF, 0x8, 0xF, 0x6, 0x3, 0x6, 0x9};
	char lut1[0x10] = {0x4, 0x1, 0xA, 0x7, 0xE, 0x5, 0xE, 0x1, 0xC, 0x9, 0x8, 0x5, 0x6, 0x3, 0xC, 0x9};
