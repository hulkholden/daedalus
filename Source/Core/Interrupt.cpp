...
inline void SetCauseRegister(u32 mask, u32 exception)
{
...
void R4300_Exception_Break()
{
	...
	SetCauseRegister(CAUSE_EXCMASK, EXC_BREAK);
...
void R4300_Exception_Syscall()
{
	...
	SetCauseRegister(CAUSE_EXCMASK, EXC_SYSCALL);
...
void R4300_Exception_CopUnusuable()
{
	...
	SetCauseRegister((CAUSE_EXCMASK | CAUSE_CEMASK), (EXC_CPU | SR_CU0));
...
void R4300_Exception_TLB(u32 virtual_address, u32 exception_code, u32 exception_vector)
{
	...
	SetCauseRegister(CAUSE_EXCMASK, exception_code);
...
void R4300_Handle_Interrupt()
{
	...
	DAEDALUS_ASSERT(mi_interrupt_set == cause_int_3_set, "CAUSE_IP3 inconsistant with MI_INTR_REG (%08x)",
	                Memory_MI_GetRegister(MI_INTR_MASK_REG) & Memory_MI_GetRegister(MI_INTR_REG));
#endif

	// Ensure ERL/EXL are "0" and IE is "1"
	if ((gCPUState.CPUControl[C0_SR]._u32 & (SR_EXL | SR_ERL | SR_IE)) == SR_IE)
	{
		// Are interrupts pending/wanted?
		if (gCPUState.CPUControl[C0_SR]._u32 & gCPUState.CPUControl[C0_CAUSE]._u32 & CAUSE_IPMASK)
		{
#ifdef DAEDALUS_PROFILE_EXECUTION
			gNumInterrupts++;
#endif
			SetCauseRegister(CAUSE_EXCMASK, EXC_INT);
